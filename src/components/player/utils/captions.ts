import { labelToLanguageCode, RunOutput } from "@/lib/providers";
...
export function convertProviderCaption(
  ...
  return captions.map((v) => ({
    id: v.id,
    language: labelToLanguageCode(v.language) ?? v.language.toLowerCase(),
    url: v.url,
    type: (v as any).type,
    ...
    flagUrl: (v as any).flagUrl,
    release: (v as any).release,
    releases: (v as any).releases,
    origin: (v as any).origin,
  }));
}
